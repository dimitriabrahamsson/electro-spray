...
# infiles
parFile = open('che.prm', 'r')
topFile = open('che.rtf', 'r')
...
topFile = open('/home/anton/Dropbox/GROMACS/Ionization/2-Amino-6-methoxybenzothiazole/2AM6/CharmmToGro/che.rtf', 'r')
	...
	if not presFlag:
		...
		if line.startswith('BOND') or line.startswith('DOUBLE'):
			...
			if firstBond:
				...
				segments = line.split()
				bondNumber = np.round(len(segments[1:])/2).astype(int)
		...
		if line.startswith('IMPR'):
			if firstImpr:
				...
				segments = line.split()
				imprNumber = np.round(len(segments[1:])/4).astype(int)
		...
		if nonbonded and not hbond:
			...
			if line[0]!=comment and segments[0]!='NONBONDED' and segments[0]!='cutnb' and segments[0]!='CUTNB':
				...
				if type == 'HT':
					...
					if type2element.__contains__(type):
